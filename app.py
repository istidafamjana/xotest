...
# التوكنات والمفاتيح - يجب تحديث هذه القيم بالقيم الصحيحة
...
try:
    ...
    logger.error(f"خطأ في تهيئة نموذج Gemini: {str(e)}")
    model = None
...
def send_message(recipient_id, message_text, buttons=None):
    ...
    try:
        ...
        logger.error(f"خطأ في إرسال الرسالة إلى {recipient_id}: {str(e)}")
        return False
        ...
        if 'text' in message:
            ...
            if user_message.lower() in ['مساعدة', 'help']:
                ...
                try:
                    ...
                    if not model:
                        send_message(sender_id, "⚠️ البوت غير متاح حاليًا، يرجى المحاولة لاحقًا")
                        return
...
@app.route('/')
def home():
    return "Facebook AI Bot - Single Threaded per User"
